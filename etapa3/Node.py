        ...
        with self.lock: 
            for neighbor_address, session_info in self.sessions.get(filename, {}).items():
                ...
                if session_info.get('flow') == "YES":
                    count_yes += 1
                    # Retorna imediatamente se encontramos mais de uma pessoa com 'flow' == "YES"
                    if count_yes > 1:
                        return True
            # Retorna False se houver 0 ou 1 pessoa com 'flow' == "YES"
        return False
    
    def at_least_one_session_with_flow(self, filename):
        count_yes = 0
        # Itera pelos endereços de vizinhos no arquivo especificado
        with self.lock: 
            for neighbor_address, session_info in self.sessions.get(filename, {}).items():
                ...
                if session_info.get('flow') == "YES":
                    ...
                    if count_yes > 0:
                        return True
            # Retorna False se houver 0 com 'flow' == "YES"
        return False
