	...
	def start_new_session(self):
		...
		while True:
			...
			if best_route:
				...
				video_session = VideoSession(session_window, destination_ip, destination_rtsp_port, self.rtp_port, self.filename)
				
				# Adiciona um botão para fechar a sessão
				close_button = Button(self.sessions_frame, text=f"Fechar Sessão {video_session}", command=lambda: self.close_session(video_session))
				close_button.pack()
